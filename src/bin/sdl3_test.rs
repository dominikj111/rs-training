...
struct AppState {
    ...
    rng: SimpleRng,                       // Random number generator
}
...
fn shoot(shooter: usize, players: &mut [Player], players_len: usize, rng: &mut SimpleRng) {
    ...
    for (i, target) in players.iter_mut().enumerate().take(players_len) {
        ...
        if hit > 0 {
            ...
            target.pos[0] = MAP_BOX_SCALE as f64 * (rng.gen_float() - 0.5);
            target.pos[1] = MAP_BOX_SCALE as f64 * (rng.gen_float() - 0.5);
            target.pos[2] = MAP_BOX_SCALE as f64 * (rng.gen_float() - 0.5);
        }
    }
}
